...
let rec pp ppf = function
  ...
  | Trait (n, p, f, w, d) ->
    Format.fprintf ppf "trait %s%a%a%a { %a } " n pp_params p pp_for f pp_with w
      pp_entities d
  | Impl ([], t, f, w, d) ->
    Format.fprintf ppf "impl %a%a%a { %a } " Type.pp t pp_for f pp_with w
      pp_entities d
  | Impl (p, t, f, w, d) ->
    Format.fprintf ppf "impl forall %a. %a%a%a { %a } " pp_params p Type.pp t
