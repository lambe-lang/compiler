...
let should_unify_apply_type () =
  ...
  and computed = unify (Apply (Variable "x", Ident "String")) (Apply (Ident "Int", Variable "y")) in
...
let should_not_unify_apply_type () =
  ...
  and computed = unify (Apply (Variable "x", Ident "String")) (Apply (Ident "Int", Variable "x")) in
  Alcotest.(check unify_type) "should_not_unify_apply_type" expected computed

let should_not_unify_apply_type_cyclic () =
  let expected = Error (CyclicUnification (Variable "x", Apply (Ident "Int", Variable "x")))
