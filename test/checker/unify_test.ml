...
let should_not_unify_arrow_type_cyclic () =
  ...
  and computed =
    Types.unify
      ...
      (Arrow (Native Int, Variable "x"))
  in
  Alcotest.(check unify_type) "should_not_unify_arrow_type_cyclic" expected computed
...
let test_cases =
  ...
  , let open Alcotest in
    ...
    ; test_case "Should not unify arrow type (cyclic)" `Quick should_not_unify_arrow_type_cyclic
