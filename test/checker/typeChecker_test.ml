...
let should_synthetize_int_type () =
  ...
  and computed = TypeChecker.synthetize () Term.(Literal (Integer 1)) in
...
let should_synthetize_float_type () =
  ...
  and computed = TypeChecker.synthetize () Term.(Literal (Float 1.)) in
  Alcotest.(check (result lambe_type string))
